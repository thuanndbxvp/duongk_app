...
├── Next.js (App Router, SSR)         ← Frontend (BFF)
├── FastAPI (Python 3.12)             ← Backend REST API + BackgroundTasks
...
- **Chạy:** Next.js + FastAPI (native BackgroundTasks) + Caddy + OmniVoice
- **Tùy chọn:** Redis (nếu cần cache/rate-limit)
...
- **Giao tiếp với CPU VPS:** qua Modal SDK / private network
...
- **CPU VPS** → web/API/orchestration/deterministic analytics + BackgroundTasks
...
- Chỉ credentials hạ tầng (Supabase URL, R2 access key) mới nằm trong `.env`
...
| Backend REST | Python + FastAPI + Pydantic v2 + BackgroundTasks | Python 3.12, FastAPI 0.115+ |
...
/apps
  ...
  /api                     # FastAPI (REST API + BackgroundTasks, không có LLM key trong env)
...
Xem hướng dẫn thao tác chi tiết tại [RUN.md](RUN.md) cùng thư mục \docs\important\.
