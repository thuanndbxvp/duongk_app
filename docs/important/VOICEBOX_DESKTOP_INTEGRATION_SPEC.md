...
>
> 💡 *Mô hình*: **Hybrid Desktop-First (Client siêu nhẹ ~25MB + Cloud GPU Modal siêu tốc)**.
...
└──────────────────────────────────────┬──────────────────────────────────────┘
                                       │ (HTTPS REST API / Streaming WAV)
                                       ▼
┌─────────────────────────────────────────────────────────────────────────────┐
│                GATEWAY & REGISTRY SERVER (https://voice.ai86.click)          │
│  - Cung cấp Catalog giọng nói (System + Cloned Voices)                      │
│  - Upload file mẫu tham chiếu (.wav, .mp3)                                   │
│  - Kích hoạt suy luận GPU không đồng bộ sang Modal.com                      │
└──────────────────────────────────────┬──────────────────────────────────────┘
...
└──────────────────────────────────────┬──────────────────────────────────────┘
                                       ...
                                       ▼
┌─────────────────────────────────────────────────────────────────────────────┐
│              CLOUDFLARE R2 OBJECT STORAGE (appdk-uploads / appdk-renders)   │
└─────────────────────────────────────────────────────────────────────────────┘
```

---

## 2. 🔌 Các Thông Số Hạ Tầng Dùng Chung (Existing Infrastructure Parameters)

Dưới đây là các thông số hạ tầng đang hoạt động mà Desktop App sẽ kết nối trực tiếp:

### A. Domain & API Endpoints
* **Voice Studio Gateway Base URL**: `https://voice.ai86.click`
* **Main API Base URL**: `https://api.ai86.click`
* **CDN Public Media**: `https://cdn.ai86.click`

### B. Modal.com Serverless Pipeline
* **Modal App Name**: `ai-dubbing-pipeline`
* **Tên các hàm Remote Function trên Modal**:
  1. `synthesize_vieneu`:
     - **Tham số**: `text` (str), `output_key` (str), `reference_audio_url` (str | None), `voice_preset` (str | None), `speed` (float = 1.0), `denoise` (bool = True).
  2. `synthesize_voice`:
     - **Tham số**: `text` (str), `output_key` (str), `reference_audio_url` (str | None), `reference_prompt_url` (str | None), `instruct` (str | None), `speed` (float = 1.0).
  3. `cache_voice_prompt`:
     - **Tham số**: `reference_audio_url` (str), `output_key` (str).
  4. `dub_srt`:
     - **Tham số**: `srt_content` (str), `output_key` (str), `voice_id` (str), `reference_audio_url` (str | None), `reference_prompt_url` (str | None), `merge_mode` (str = "native"), `engine` (str = "vieneu").

### C. Cloudflare R2 Buckets
* `appdk-uploads`: Chứa file audio mẫu (`voices/`, `omnivoice_refs/`).
* `appdk-renders`: Chứa file audio sinh ra (`omnivoice_renders/`).
* `appdk-cache`: Chứa cache prompt (`omnivoice_prompts/`).

---

## 3. 📡 Chi Tiết API Specs Dành Cho Desktop Client

Desktop App (Voicebox) chỉ cần gọi các endpoint REST tiêu chuẩn sau của Gateway `https://voice.ai86.click`:
...
### 2. Sinh giọng đọc Text-to-Speech (TTS)
* **Method**: `POST`
...
```
* **Ghi chú về `engine`**:
  * `"vieneu"` *(Khuyên dùng cho Tiếng Việt)*: Sử dụng VieNeu-TTS v3 Turbo. Hỗ trợ các tag biểu cảm: `[cười]`, `[thở dài]`, `[hắng giọng]`.
  * `"omnivoice"`: Sử dụng OmniVoice (Hỗ trợ Voice Design tự do qua trường `"instruct"`).
* **Response**: Trực tiếp Binary Stream của file âm thanh (`Content-Type: audio/wav`). Desktop App có thể phát trực tiếp hoặc lưu ra cache file `.wav`.
...
```
* **Response**: `{"id": "bac_ba_ke_chuyen", "display_name": "Bác Ba Kể Chuyện", ...}` (Hệ thống sẽ tự động cache prompt sang Modal và lưu trữ vĩnh viễn).
...
* **Form Data**:
  ...
  * `speed`: `1.0`.
* **Response**: Binary Stream file audio hoàn chỉnh đã khớp timecode (`audio/wav`).

---

## 4. 🛠️ Hướng Dẫn Từng Bước Cho AI Coding (Implementation Guide)
...
### Bước 2: Tích hợp Engine Selector
...
### Bước 3: Cơ chế Cache Audio Local trên Desktop
* Khi Desktop nhận được stream WAV từ `https://voice.ai86.click/v1/tts`, lưu tạm vào thư mục cache của OS:
  * Windows: `%APPDATA%/voicebox-ai86/cache/`
  * macOS: `~/Library/Caches/voicebox-ai86/`
...
* Sử dụng lệnh build tiêu chuẩn của Tauri:
  ...
  npm run tauri build
  ```
* Kết quả thu được file installer `.msi` / `.exe` (Windows) hoặc `.dmg` (macOS) có dung lượng chỉ khoảng **15 - 30 MB**, sẵn sàng phát hành cho khách hàng.

---

## 5. ✅ Checklist Kiểm Thử (Verification Criteria)

1. [ ] Khởi động Desktop App không báo lỗi thiếu PyTorch/CUDA local.
2. [ ] Gọi `GET https://voice.ai86.click/v1/catalog` tải danh sách giọng thành công.
3. [ ] Nhập văn bản tiếng Việt, bấm Generate -> Nhận âm thanh WAV phát rõ ràng trong vòng **1-3 giây**.
4. [ ] Kéo thả file âm thanh mẫu để clone giọng -> Giọng mới xuất hiện ngay trên Desktop UI.
5. [ ] Đóng app, mở lại -> Danh sách giọng clone vẫn tồn tại nguyên vẹn.
